...
// copier fonction spécifier ici (core-utils)ouiiiiiiiiiiiiiiiiiiiiiiiiiiiiii
import {
  createCamera,
  createRenderer,
  runApp,
...
let camera = createCamera(60, 1, 60, { x: 0, y: 0, z: 2.5 });
...
let app = {
  vertexShader() {
    ...
    float noise (in vec2 st) {
        ...
        // Cubic Hermine Curve.  Same as SmoothStep() change en cube 
    ...
    void main() {
      ...
      vec3 pos = position;
      pos.z += noise(pos.xy * u_noise_freq_1 + u_time * u_spd_modifier_1) * u_noise_amp_1;
      pos.z += noise(rotate2d(PI / 4.) * pos.yx * u_noise_freq_2 - u_time * u_spd_modifier_2 * 0.8) * u_noise_amp_2;

      vHeight = pos.z; // Pass height to the fragment shader
      vec4 mvm = modelViewMatrix * vec4(pos, 1.0);
      gl_Position = projectionMatrix * mvm;
    }
  ...
  fragmentShader() {
    ...
    void main() {
      // Normalize height to range [0, 1] (assuming max height is 1.0 for simplicity)
      float normalizedHeight = clamp(vHeight, 0.0, 1.0);

      // Interpolate between white and #062746
      vec3 colorLow = vec3(1.0); // White
      vec3 colorHigh = vec3(0.024, 0.153, 0.275); // #062746 in RGB
      vec3 color = mix(colorLow, colorHigh, normalizedHeight);
  ...
  async initScene() {
    ...
    this.controls.enabled = false; // This completely disables all OrbitControls interactions
    ...
    // Mesh
    this.geometry = new THREE.PlaneGeometry(3.5, 3.5, 1024, 1024);  // si tu change les 2 dernières valeur, ca te permet d'augmenter le nombre de points
                                                              // par default 3ème valeur=128 et 4ème valeur=128
                                                              // 3ème valeur = nombre de points par lignes
                                                              // 4ème valeur = nombre de points par colonnes
